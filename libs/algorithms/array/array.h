...
size_t binary_search_more_or_equal_ (const int *a, const size_t n, int x) ;
...
void delete_by_pos_save_order_ (int *a, size_t *n, size_t pos) ;
...
void delete_by_pos_unsave_order_ (int *a, size_t *n, size_t pos) ;

// возвращает значение ’истина’ если все элементы
// массива data размера n соответствует функции-предикату predicate, иначе - ’ложь’
...
void for_each_ (const int* source, int *dest, size_t n, const int (*predicate)(int));

// возвращает количество элементов массива data размера n удовлетворяющих функции-предикату predicate
int count_if_ ( const int *a, size_t n, int (* predicate ) (int)) ;
...
void delete_if_ (int *a , size_t *n, int (* deletePredicate ) (int)) ;
